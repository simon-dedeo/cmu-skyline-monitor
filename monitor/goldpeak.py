...
ROI_TOP = 0.62          # sky region = top 62% (matches expose.py sky ROI)
BRIGHT_FLOOR = 50       # full-frame mean; reject near-black twilight frames
TARGET_ELEV = -1.0      # choice point: sun 1 deg below horizon (Simon 2026-07-16)
...
# Sky sample = the "top-middle sixth" (3-col x 2-row grid cell): central upper sky.
# Avoids corner skyglare / window reflections at the frame edges while keeping plenty
# of pixels for a stable average. Used for both the golden hue and the colour bar.
SKY_ROWS, SKY_COLS = (0.0, 0.5), (1.0 / 3, 2.0 / 3)
...
CAMPUS_ROW = 0.55   # rows below this = "campus" (buildings/lawn, no sky) — keep in
                    # step with science.py CAMPUS_ROW; re-tune if the cam is re-aimed


def campus_hue_full(img):
    """Hue of everything BELOW the skyline (rows 55-100%) — campus colour.
...
def main():
    ...
    score = -abs(sun_elev_now - TARGET_ELEV)
