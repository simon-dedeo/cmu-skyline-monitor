...
# Rolling window (Simon 2026-07-27): the fit used to run cumulatively over the whole
# archive, so a monotonic aim drift showed up as a slow ramp in t. WINDOW_DAYS keeps
# each night's estimate local in time; drift within the window is handled by the
# per-tick aperture recentring in spotlab.py, not by this filter.
WINDOW_DAYS <- as.numeric(Sys.getenv("SPOTFIT_WINDOW_DAYS", "3"))
cutoff <- as.POSIXct(Sys.time(), tz = "UTC") - WINDOW_DAYS * 86400
d$.ts <- as.POSIXct(substr(d$ts_utc, 1, 19), format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
d <- d[!is.na(d$.ts) & d$.ts >= cutoff, ]
say("window: last %.1f d (cutoff %s) -> %d rows", WINDOW_DAYS,
    format(cutoff, "%Y-%m-%dT%H:%M:%SZ"), nrow(d))
