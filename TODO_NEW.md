...
**TL;DR status (updated 2026-07-20):** System is live and healthy on `pandr` with the
**Elgato Facecam 4K attached and fully integrated.** Every 5 min the tick captures an
**HDR-fused DISPLAY frame** (`sky.jpg`) + a **fixed-exposure SCIENCE reference**
(`sky_ref.jpg`), runs the science log, and the golden-hour scanner runs its own HDR peak
capture during golden windows. Big 4K images are hosted on **ganesha
(photos.proofsandreasons.io)**; small weather/colour data still goes to santafe. The
public site (proofsandreasons.io) shows weather + the golden image + a dashboard.

> ⚠️ **Parts 0 and 4 below were written pre-camera (2026-07-17) and describe the old
> "no camera yet" state and the ssh-hop capture model — both SUPERSEDED.** Read
> **"Camera & exposure system (2026-07-20)"** just below Part 0 for how capture actually
> works now (SkyCam.app + uvc-util + skyshot.sh; no ssh hop; `expose.py` retired).
...
|---|---|---|
| `com.lsm.monitor` | 300 s | `tick.sh` → `capture.sh` (**NO ssh hop**) → `skyshot.sh hdr` (sky.jpg, display) + `skyshot.sh ref` (sky_ref.jpg, science) → `update.py` (weather/solar/health) → archives → `science.py`. Live 4K sky.jpg rsync'd to ganesha; small data to santafe (~15 min throttle) |
| `com.lsm.goldscan` | 150 s | `goldtick.sh` → `goldscan.sh` (**NO ssh hop**); no-op outside golden windows; inside them captures an HDR frame, scores the peak (nearest sun elev −1°), uploads once at window close to santafe **and** ganesha |
...
- **Camera + launchd TCC — SOLVED via SkyCam.app (2026-07-20, Sequoia).** Bare CLI tools
  get NO camera on Sequoia (silently denied, no prompt). We capture through
  `~/monitor/SkyCam.app` — a signed Swift/AVFoundation still-grabber carrying
  NSCameraUsageDescription — launched via `open -W` so TCC attributes the grant to the
  app itself. The old `ssh localhost` hop is **RETIRED** (it was a Big-Sur imagesnap
  trick; it does NOT grant camera on Sequoia). Manual exposure/gain/WB come from
  `~/monitor/uvc-util` (raw UVC control transfers — not TCC-gated). Details below.
...
**Key files in `~/monitor` (2026-07-20):** capture stack = `SkyCam.app` (signed
AVFoundation still-grabber, source `skycap.swift`) + `uvc-util` (UVC exposure/gain/WB
control) driven by **`skyshot.sh`** (modes `auto`|`ref`|`hdr`), with **`skyexp.py`**
(sun-elevation → exposure/gain table + HDR bracket) and **`hdrfuse.py`** (Mertens
exposure fusion). `tick.sh` → `capture.sh` = the 5-min tick; `update.py` = data.json;
`goldpeak.py`/`goldscan.sh`/`goldtick.sh` = golden-hour; `science.py` = per-tick metrics
→ science.csv; `solar.py` = sun/golden times; `aimer.sh` (+ `aim.html`) = the live kiosk
aimer; `dashboard.html`, `roi_overlay.py`, `drift.py`. **RETIRED with the old camera**
(kept only for reference, NOT run): `expose.py`, `aim.py`, `usbreset.py`, `experiment.py`,
`hdr_test.py`, `timelapse.sh`, and `tick.sh`'s old ssh-hop. Old-machine data parked in
`~/monitor/OLD-MACHINE-DATA-2026-07-17/`. Adapted-code mirror on Simon's laptop:
`/Users/simon/Desktop/MBP/monitor-pandr/`.

---

## Camera & exposure system (2026-07-20) — READ THIS

> **Window-spot program (2026-07-20):** a fixed blemish on the window (~2% dip at (1163,187))
> is under active multi-day characterization — see **SPOT.md** for the full plan (Bayesian
> t/s model, ladders, control patches, re-enable gates). Correction in served images is
> **disabled** (`~/monitor/.spot_apply_on` absent) until the model passes validation.
> Per-tick data gathering (`spot_lab/`), aim-drift (`drift.csv`), and daily calibration
> ladders are live.

**Camera:** Elgato Facecam 4K (USB `0x0fd9:0x0092`, AVFoundation name "Elgato Facecam 4K",
3840×2160). Roughly aimed at the CMU columned building (left) + modern building (right) +
terraced plaza/lawn; blue sky in the top ~35%.

**Capture stack (three layers):**
1. **`SkyCam.app`** — signed Swift/AVFoundation bundle (`skycap.swift`) that grabs ONE
   full-res 4K JPEG. Launch via `open -W -a ~/monitor/SkyCam.app --args --device "Elgato
   Facecam 4K" --out ABS_PATH --warmup N`. This is the ONLY thing that gets camera TCC on
   Sequoia. Signed with a stable self-signed cert ("SkyCam Signer") so rebuilds don't
   re-prompt. Also has `--stream` (1080 live preview, used by the aimer).
2. **`uvc-util`** (`~/monitor/uvc-util`, built from jtfrey/uvc-util via clang) — sets UVC
   controls over IOKit (NOT TCC-gated): `auto-exposure-mode` (1=manual, 8=auto/aperture),
   `exposure-time-abs` (1–1000 = 0.1 ms–100 ms), `gain` (0–160), `white-balance-temp`
   (2400–10000 K) + `auto-white-balance-temp`. Verified: a manual exposure set here is
   honoured by the next SkyCam capture (AVFoundation does NOT reset it).
3. **`skyshot.sh MODE OUT`** — orchestrates the two above. Modes:
   - `auto` — auto-exposure + auto-WB (live look; used by the aimer path).
   - `ref`  — FIXED radiometric reference: manual, sun-elevation-keyed exposure+gain
     (`skyexp.py`), fixed daylight WB 5600 K. Same elevation ⇒ comparable brightness, so
     cloud/haze shows as real signal. Writes `OUT` + `OUT.meta` (exposure,gain,wb,elev).
   - `hdr`  — 3-exposure bracket (±2 stops around the reference) → Mertens fusion
     (`hdrfuse.py`) → one tone-mapped DISPLAY JPEG. Fixed WB keeps golden warm.
   Always restores the camera to AUTO on exit (trap) and writes atomically (temp+rename).

**What the tick produces** (`capture.sh`, every 5 min): `sky.jpg` = HDR display frame
(→ kiosk + ganesha/photos.proofsandreasons.io) and `sky_ref.jpg` = fixed reference
(→ `archive_ref/`, read by `science.py` for ALL metrics + colour swatches). During a
golden window `goldscan.sh` owns `sky.jpg` (its own HDR peak); the 5-min tick then only
refreshes `sky_ref.jpg` so the science series stays continuous.

**Calibration TODO (needs multi-day data — the one real open item):** `skyexp.py TABLE`
maps sun elevation → (exposure, gain). Daytime rows are MEASURED (clear sky, elev ~40°:
E≈11 → mean ~120); the **dawn/dusk/night rows are physics-seeded guesses.** Check
`science.csv` `ref_exp`/`bright`/`sky_hex` through real twilight and adjust so reference
frames are neither clipped nor black. Also consider: lossless-PNG reference (SkyCam
outputs JPEG today — Part 7 storage plan wants PNG ref) and a single-session SkyCam
bracket mode (HDR is 3 separate `open` launches ≈ 14 s/tick; fine, but optimisable).
...
## Part 4 — Camera bring-up checklist — ✅ DONE 2026-07-20

*Camera arrived, aimed, and fully integrated (see "Camera & exposure system" above).
TCC solved via SkyCam.app; manual exposure via uvc-util; science ROIs re-tuned to the
new framing (`science.py`/`goldpeak.py`); HDR display + fixed reference wired into the
tick. Remaining: the aim-drift check (step 7 below — still ongoing) and the twilight
exposure-table calibration. Original checklist kept for reference:*
