...
\title{\bfseries Pittsburgh in a Grain of Sand\\[0.4em]
  \large\mdseries Lessons for Mars rovers and autonomous self-driving cars}
\author{CMU Skyline Monitor --- \code{pandr} / \code{akdeniz}\thanks{Operated by
...
\begin{abstract}
\noindent
Every camera that looks through glass eventually encounters a speck of dirt. A grain
on a window in Porter Hall at Carnegie Mellon --- consistent with 0.25 millimetres
of opaque debris, three centimetres before a sky monitor's $f/4$ pupil --- prints a
76-pixel shadow on every image, and, measured closely enough, becomes a
single-particle optics experiment. With the pupil 7--10 times wider than the grain's
forward-diffraction lobe, the system operates in the wide-acceptance limit of the
extinction paradox: shadow depth sits at the projected-area value (4.5--5.4 per cent
against $\approx$5.2), the forward-scattered half recollected rather than registered
as extinction, and recollected blue scatter approximately predicts the green channel
from blue and red alone --- measured extinction is a property of the
instrument--particle pair, not of the particle. The shadow also acts as an
(as yet uncalibrated) strain gauge for millimetre-scale flexure of the window sash;
no motion of the grain itself is required. The geometry reproduces the dust-shadow
optics developed for the Mars rovers' cameras, extended with per-channel photometry
and continuous tracking: to our knowledge no prior work continuously tracks the
apparent image position of a single adherent near-lens occluder. An autonomous
pipeline --- a recursive, gated tracker validated offline against a Bayesian pixel
model --- erases the artifact from served images (median dip 4.5 to 0.5 per cent,
retrospectively evaluated; live at
\href{https://proofsandreasons.io}{proofsandreasons.io}), and motivates
physics-first handling of the soiled, distrusted cameras of self-driving cars. To
provide information to the wider scientific community, raw data, materials, methods,
and analysis code are available at
\href{https://github.com/simon-dedeo/cmu-skyline-monitor}{github.com/simon-dedeo/cmu-skyline-monitor}.
\end{abstract}
...
\begin{center}
  ...
  the correction --- 2026-07-25 19:17 EDT. The blemish is the dark smudge in the red square at the top of
...
night's accepted state (a point estimate with gates, not a posterior) propagated by
the blemish's own observed velocity (self-calibrating
...
\item \textbf{Persistent template lock} --- the measurement update. The corrector
  ...
  (normalised correlation $\ge 0.45$, unambiguous runner-up margin, bounded
  displacement) snaps the map onto the measured position \emph{and is saved};
  ring cleanliness gates the amplitude self-calibration, not the lock. A lock must also be unambiguous: the runner-up
  ...
  a torn or corrupt publish is rejected and the corrector falls back to its
  self-referential ring fit for that tick (the last accepted bundle is not cached in
  memory). Every tick appends its provenance --- model timestamp, map hash, code
  version, enable-flag state, lock diagnostics --- to \code{spot\_provenance.csv}.
\end{enumerate}
...
bound nor get eaten by it, and the grain's noise is never explicitly filtered; the multiplicative correction
rescales local noise amplitude together with the signal, by design.
...
to the gain that nulls the \emph{fused} dip, and refine once. Because the gain is chosen by evaluating the corrected residual on the frame itself, the archive numbers it produces are an oracle-style upper bound, reported separately from the live path. This removes all dependence on the
...
motion of the object at all: at $\sim\!\SI{3}{cm}$ standoff the translational lever is
$f_{\mathrm{px}}/d \approx$ \SIrange{63}{73}{px/mm} on axis (focal length
$f_{\mathrm{px}} \approx \SI{2200}{px}$ from the advertised \ang{90} diagonal field of
view; $d =$ \SIrange{30}{35}{mm}), rising toward $\sim$\SIrange{75}{87}{px/mm} at the
blemish's off-axis position. The week's \SI{120}{px} walk is then
\SIrange{1.4}{1.9}{mm} of cumulative flexure and the intraday motion
\SIrange{0.08}{0.3}{mm} --- unremarkable for a sun-struck sash with a camera resting
against it. These are uncalibrated estimates: the local plate scale at the blemish has
not been measured, and a controlled sub-millimetre camera translation (\S\ref{sec:limitations})
is the direct calibration. (An earlier draft quoted \SI{92}{px/mm} from an
average-pixels-per-degree shortcut that overstates the paraxial scale; a referee's
recomputation prompted this correction.) Day-by-day step ratios ranging from
...
\midrule
nightly estimator (pinned centroid, recursive prior) & \code{akdeniz} & nightly & seconds \\
MAP position/shape fit (multi-start L-BFGS; offline validation) & \code{akdeniz} & on demand & seconds \\
...
Paired evidence is retained in machine-readable form
(\code{spot\_paper/evidence/live\_paired.csv}: per tick, the archived served frame
vs.\ a re-fusion of its own raw brackets uncorrected, with positions, levels, ring
structure and model version). A referee correctly observes that these rows are
\emph{retrospective}: the analysis script stamps each row with the model current at
analysis time, not the bundle that served the tick, and archive frames may have been
reprocessed --- so the comparison establishes paired-reprocessing performance, not
contemporaneous live performance. Per-tick provenance (model hash, code version,
enable state, lock diagnostics; \code{spot\_provenance.csv}) now accumulates
append-only on the capture host, and a prospective frozen-model trial over complete
clear, mixed and cloudy days is the planned test. On the same clean-tick selection as Table~\ref{tab:strata}: the
...
size $s$ removes the fraction $(s/a)^2$ of a pixel's light and images with a support
width $\approx\big(s + a(1-d/F)\big)/d$ (a support scale, not automatically the
measured FWHM; the difference is absorbed into the fitted range of $d$, and the
pixel-bundle factor $1-d/F$ assumes the nominal \SI{0.5}{m} focus conjugate of a
spec-sheet focus \emph{range}). The measured depth (\SIrange{3}{7}{\percent}) gives
...
motion: image shift equals camera--pane translation times the local plate scale over
$d$ --- \textbf{\SIrange{63}{87}{px/mm}}, uncalibrated (\S\ref{sec:physics}). The
feature's entire week of ``migration'' (\SI{120}{px}) is \SIrange{1.4}{1.9}{mm} of
accumulated flexure; its intraday drift is \SIrange{0.08}{0.3}{mm} --- the thermal
breathing of a sun-struck sash with a camera resting against it. No motion on the
glass is required, although only a fixed fiducial can prove none occurred. The kinematic anomalies that
...
secondary spectrum, edge diffraction at Fresnel number $\sim$1 (radius convention), sensor carrier
...
$\theta_{\mathrm{acc}} \approx a/(2d) \approx \SI{18}{mrad}$ half-angle
(\SI{34}{mrad} full cone) re-enters the pupil. The
...
\textbf{neutral to $\pm 8\%$}. These are consumer-camera bands after demosaicing, white balance and colour
processing, linearised by an assumed inverse-sRGB curve; the integrals are computed
within $r<\SI{90}{px}$ with negative map values clipped, and carry no formal
uncertainty --- the channel comparison is indicative, not calibrated. Subject to that
caveat, the bound disfavours strongly pigmented candidates: rust ($\times$ several
...
\paragraph{Morals.} Three. First, \emph{an assumed instrument
...
ours closely --- but their quantitative validation compared artifact \emph{diameters};
reference-particle variability prevented a predicted-vs-measured optical-density test.
Our depth photometry is therefore complementary evidence in the same geometry rather
than a confirmation of their quantitative predictions, with two extensions their
achromatic, deposition-only setting did not need: per-channel photometry and
continuous position tracking of the apparent artifact position.
...
\emph{not removed}, and theory predicts it should register an effective attenuation
at geometric blockage, not the asymptotic $Q_{\mathrm{ext}} = 2$. It does: the
inspected $\sim\!\SI{0.25}{mm}$ speck predicts $(s/a)^2 = 5.2\%$ obstruction; the
measured per-channel core depths run \SIrange{4.5}{5.4}{\percent}
(Table, \S\ref{sec:what}), where doubling would give $10.3\%$. We do not claim this
\emph{excludes} $Q_{\mathrm{ext}}=2$ --- $Q_{\mathrm{ext}}$ is a far-field
plane-wave cross-section, and the near-field, wide-acceptance conditions here are
precisely those in which the paradoxical factor is recollected rather than
registered; the measurement instantiates that regime, it does not adjudicate the
cross-section. (Markel's $\sim\!60^\circ$ convergence criterion likewise concerns a
plane-wave integrating-detector configuration; we use it as an order-of-magnitude
guide, not an exact correspondence.)
...
is then \emph{predicted} at a relative integral of $1.045$ (no formal uncertainty is propagated through the maps and colour pipeline --- a day-level bootstrap is future work); it is measured at
...
the \emph{apparent image position of a single adherent occluder} --- the shadow
migrates across the frame whether or not the grain moves on the glass ---
...
\paragraph{A gap opened: the shadow as metrology.} The \SIrange{63}{87}{px/mm} (uncalibrated) lever that
...
remains future work; the synthetic test now exits nonzero on failure. The full
repository, including a raw-bracket sample sufficient to reproduce
Figure~\ref{fig:full} and the deployed code as byte-copies, is public at
\href{https://github.com/simon-dedeo/cmu-skyline-monitor}{github.com/simon-dedeo/cmu-skyline-monitor};
the complete bracket archive ($\sim$hundreds of GB) is available on request. A
point-by-point response to the second review is \code{REFEREE2\_ACTIONS.md}.
...
\item One-minute golden-hour frames keep no raw brackets, so their era corrections
  ...
  frames.
\item The plate scale at the blemish is uncalibrated; all flexure figures inherit a
  $\sim$30\% scale uncertainty until a controlled camera-translation calibration is
  done. The colour-channel comparison is likewise uncalibrated (consumer pipeline,
  assumed inverse-sRGB); narrow-band or RAW measurements would make it quantitative.
\item The physical identification is a consistent solve, not a controlled
  measurement: known-size calibration dots, a measured entrance pupil, a fixed pane
  fiducial and the wipe test (\S\ref{sec:what}) are the decisive experiments, none
  yet performed.
\item Paired performance evidence is retrospective (see \S\ref{sec:results}); no
  false-correction study under contrails, birds, glare or saturation has been run.
\end{itemize}
