...
\usepackage[colorlinks=true,linkcolor=blue!55!black,urlcolor=blue!55!black,
            ...
            pdftitle={Tracking and Removing a Migrating Window Blemish in a Time-Lapse Sky Monitor},
...
\begin{abstract}
\noindent
The skyline monitor observes through a window carrying a $\sim\SI{0.25}{mm}$ debris
speck, imaged by the camera's $f/4$ pupil from $\sim\SI{3}{cm}$ as a \SI{76}{px},
\SIrange{2}{7}{\percent} dark smudge that exposure fusion and local-contrast enhancement
amplify $\sim\!2.9\times$ in the served image. The image of the speck is not stationary:
the shadow geometry converts sub-millimetre thermal flexure of sash and mount into
$\sim\SI{92}{px/mm}$ of image motion, so the feature walked \SI{120}{px} in a week,
including \SIrange{7}{18}{px} within single days. This note describes, and distinguishes
explicitly, three layers of work: a \emph{deployed} per-tick corrector (empirical
transmittance map, velocity placement from the data's epoch, persistent template-lock
measurement update, gated amplitude self-calibration, capped division); a
\emph{deployed} nightly estimator and publisher (recursive-prior centroid tracking,
registered map building, acceptance gating); and \emph{offline} analyses (a Bayesian
pixel-model MAP fit that validates the production tracker, and a closed-loop
reprocessing of the archive). On the disclosed offline population --- clean daylight
frames with a detectable dip --- the median displayed dip falls from \SI{4.46}{\percent}
to \SI{0.52}{\percent} (median removal \SI{88}{\percent}, day-blocked 95\% CI
\SIrange{86}{91}{\percent}, $n=495$ over 8 days). Retained live paired measurements are
smaller and more variable (\S\ref{sec:results}); the principal limitations are the
shared-centre validation geometry and one day of post-deployment evidence.
\end{abstract}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/full_before.jpg}\\[2pt]
  \includegraphics[width=\textwidth]{figs/full_after.jpg}
  \caption{The full frame, before (top) and after (bottom) correction. Both images are
  fused from the \emph{same three raw brackets} (2026-07-25 19:17:28 EDT): the top is
  the uncorrected fusion, the bottom the corrected one produced by the archive
  reprocessing --- and now installed as the served frame in the public archive. The
  blemish is the dark smudge at the top of the sky, left of centre; everything else,
  including the cloud haze at left and the crane, is pixel-identical scene. Measured
  displayed dip: \SI{5.46}{\percent} $\rightarrow$ \SI{0.77}{\percent}.
  Figure~\ref{fig:ba} shows the correction at pixel scale.}
  \label{fig:full}
\end{figure}
